 ...
 */
async function generateMCPServer(spec, config) {
...
async function generateMCPPackageJson(spec, config, outputDir) {
...
async function generateMCPTsConfig(outputDir) {
    const tsConfig = {
        compilerOptions: {
            target: 'ES2022',
            module: 'NodeNext',
...
async function generateMCPServerTs(spec, config, outputDir) {
    const srcDir = path.join(outputDir, 'src');
    const tools = (0, openapi_1.extractMCPTools)(spec);
    const resources = (0, openapi_1.extractMCPResources)(spec);
      ...
      messages: [
        {
          ...
          content: {
            type: 'text',
            text: \`I want to explore the ${spec.info.title} API.\${args?.endpoint ? \` Focus on the \${args.endpoint} endpoint.\` : ''}\n\nAvailable endpoints:\n${(0, openapi_1.getAllOperations)(spec).map(op => `- \${op.method.toUpperCase()} \${op.path}: \${op.operation.summary || ''}`).join('\n')}\`,
          },
        },
      ],
    };
...
async function generateMCPTools(spec, config, outputDir) {
    const srcDir = path.join(outputDir, 'src');
    const tools = (0, openapi_1.extractMCPTools)(spec);
...
async function generateMCPResources(spec, config, outputDir) {
    const srcDir = path.join(outputDir, 'src');
    const resources = (0, openapi_1.extractMCPResources)(spec);
...
async function generateMCPTypes(spec, config, outputDir) {
    const srcDir = path.join(outputDir, 'src');
    let content = `/**
 * MCP Server Types
...
async function generateMCPREADME(spec, config, outputDir) {
    const serverName = config.name || spec.info.title.toLowerCase().replace(/\s+/g, '-');
...
${(0, openapi_1.extractMCPTools)(spec).map(tool => `- **${tool.name}**: ${tool.description}`).join('\n')}

## Available Resources

${(0, openapi_1.extractMCPResources)(spec).map(r => `- **${r.uri}**: ${r.description}`).join('\n')}
...
function getOperationForTool(spec, operationId) {
