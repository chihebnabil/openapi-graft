...
exports.generateRustSDK = generateRustSDK;
 ...
 * Generate Rust SDK
 */
async function generateRustSDK(spec, config) {
...
async function generateCargoToml(spec, config, outputDir) {
...
async function generateLibRs(spec, config, srcDir) {
...
async function generateRustClient(spec, config, srcDir) {
...
async function generateRustModels(spec, config, srcDir) {
...
function generateRustStruct(name, schema) {
    ...
    for (const [propName, propSchema] of Object.entries(properties)) {
        const rustTyp = rustType(propName, propSchema, required.has(propName));
...
function generateRustEnum(name, schema) {
...
function rustType(name, schema, required) {
    let typeStr;
    if ('$ref' in schema) {
        ...
        switch (schema.type) {
            ...
            case 'array':
                if (schema.items) {
                    const itemType = rustType(`${name}_item`, schema.items, true);
...
async function generateRustAPI(spec, config, srcDir) {
...
function generateRustMethod(operation, method, path) {
...
async function generateRustErrors(srcDir) {
...
async function generateRustREADME(spec, config, outputDir) {
    const crateName = config.package?.replace('@', '')?.replace('/', '-') || toSnakeCase(spec.info.title) + '-sdk';
