...
exports.performThreeWayMerge = performThreeWayMerge;
 ...
 */
async function performThreeWayMerge(baseDir, oursDir, theirsDir, language, outputDir) {
    const results = [];
 ...
 */
async function mergeSingleFile(file) {
    ...
    const annotations = (0, parser_1.extractAnnotations)(file.ours, file.language);
    ...
    const [baseAST, oursAST, theirsAST] = await Promise.all([
        (0, parser_1.parseSource)(file.base, file.language, file.path),
        (0, parser_1.parseSource)(file.ours, file.language, file.path),
        (0, parser_1.parseSource)(file.theirs, file.language, file.path),
    ]);
    ...
    for (const annotationInfo of annotations) {
        ...
        try {
            const applyResult = await applyAnnotation(mergedOutput, file.base, file.ours, annotation, blockText, file.language);
 ...
 */
async function applyAnnotation(currentOutput, base, ours, annotation, blockText, language) {
 ...
 */
function applyPreserveAnnotation(currentOutput, ours, annotation, blockText, language) {
 ...
 */
function applyReplaceAnnotation(currentOutput, ours, annotation, blockText, language) {
 ...
 */
function applyExtendAnnotation(currentOutput, ours, base, annotation, blockText, language) {
 ...
 */
function formatPreservedBlock(annotation, blockText, language) {
    const commentPrefix = {
 ...
 */
function findAnnotationLine(source, annotation) {
 ...
 */
function findInsertionPoint(output, annotation, blockText, language) {
 ...
 */
function findBlockEnd(lines, startLine, language) {
    if (language === 'python') {
 ...
 */
function getBlockSignature(blockText, language) {
 ...
 */
function findBlockRange(lines, signature, language) {
    for (let i = 0; i < lines.length; i++) {
 ...
 */
function mergeBlocks(existingBlock, customBlock, language) {
 ...
 */
function extractBlockContent(blockText, language) {
    const lines = blockText.split('\n');
    if (language === 'python') {
 ...
 */
function getIndentation(blockText) {
    const lines = blockText.split('\n');
 ...
 */
function createConflict(annotation, baseText, oursText, theirsText, blockText, reason) {
    return {
        block: {
            annotation,
 ...
 */
async function findSourceFiles(dir, language) {
