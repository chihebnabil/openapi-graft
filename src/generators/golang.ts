 ...
 * Generate Go SDK
 */
export async function generateGoSDK(
  spec: OpenAPISpec,
  config: SDKConfig
): Promise<void> {
...
async function generateGoMod(spec: OpenAPISpec, config: SDKConfig, pkgName: string, outputDir: string): Promise<void> {
...
async function generateGoClient(spec: OpenAPISpec, config: SDKConfig, pkgName: string, outputDir: string): Promise<void> {
...
async function generateGoModels(spec: OpenAPISpec, config: SDKConfig, pkgName: string, outputDir: string): Promise<void> {
...
function generateGoStruct(name: string, schema: SchemaObject): string {
  ...
  for (const [propName, propSchema] of Object.entries(properties)) {
    const goTyp = goType(propName, propSchema as SchemaObject, required.has(propName));
...
function generateGoEnum(name: string, schema: SchemaObject): string {
...
function goType(name: string, schema: SchemaObject, required: boolean): string {
  let type: string;
  ...
  if ('$ref' in schema) {
    ...
    switch (schema.type) {
      case 'string':
        if (schema.format === 'date-time') type = 'time.Time';
        else type = 'string';
        break;
      case 'integer':
        if (schema.format === 'int64') type = 'int64';
        else type = 'int';
        break;
      case 'number':
        type = 'float64';
      ...
      case 'array':
        if (schema.items) {
          const itemType = goType(`${name}_item`, schema.items as SchemaObject, true);
          type = `[]${itemType}`;
        } else {
          type = '[]interface{}';
        }
...
async function generateGoAPI(spec: OpenAPISpec, config: SDKConfig, pkgName: string, outputDir: string): Promise<void> {
...
function generateGoMethod(operation: OperationObject, method: string, path: string, tag: string): string {
...
async function generateGoErrors(pkgName: string, outputDir: string): Promise<void> {
  let content = `package ${pkgName}\n\n`;
  content += `import "fmt"\n\n`;
...
async function generateGoREADME(spec: OpenAPISpec, config: SDKConfig, outputDir: string): Promise<void> {
...
function toGoPackageName(title: string): string {
